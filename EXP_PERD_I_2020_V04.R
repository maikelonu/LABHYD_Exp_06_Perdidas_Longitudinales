...
setwd("C:/Users/TEC/Google Drive/FUNDA_R_Private/R_LABHYD/EXP_PERD")
#setwd("/media/maikel/Trabajo/R_ITC/R_LABHYD/EXP_PERD")
...
df.obs <- read.table("Observaciones_Test_2020.txt", header = TRUE)
...
fg01 <- ggplot() +
  ...
  geom_abline(data=df.cup18,size = 0.95,linetype = 2) +
...
df.subset.cup18 <- subset(df.obs, pipe == "cup18" & feeding == "Tank_Flux_Dig")
df.subset.pvc12 <- subset(df.obs, pipe == "pvc12")
df.subset.pvc18 <- subset(df.obs, pipe == "pvc18")
df.subset.cup18.02 <- subset(df.obs, pipe == "cup18" & feeding == "Tank_Rot_Dig")
...
# Custom fuction f.opti is manually evaluated
...
# Simple boxplots are manually requested to deal with outliers using ggplot2
