...
setwd("C:/DATOS/R_ITC/R_LABHYD/EXP_PERD")
...
df.obs <- read.table("observaciones02.txt", header = TRUE)
...
df.subset.cup18 <- subset(df.obs, pipe == "cup18" & feeding == "E01")
df.subset.pvc12 <- subset(df.obs, pipe == "pvc12")
df.subset.pvc18 <- subset(df.obs, pipe == "pvc18")
df.subset.cup18.02 <- subset(df.obs, pipe == "cup18" & feeding == "E02")
